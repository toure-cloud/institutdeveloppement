...
from .models import Formation, UE, Activite, Inscription
...
def is_candidat(user):
    """Vérifie si l'utilisateur est un candidat (non staff)."""
...
def candidat_login_page(request):
    """Connexion spécifique pour les candidats avec affichage modal en cas d'erreur."""
    next_url = request.GET.get('next', 'espace_candidat')  # URL de redirection après login
    ...
    if request.method == 'POST':
        ...
        if user is not None:
            if not user.is_staff:  # Vérifie que ce n'est pas un compte admin
                login(request, user)
        ...
        # Si échec, conservez le next pour le réafficher dans le formulaire
        next_url = request.POST.get('next', next_url)

    return render(request, 'candidat/candidat_login.html', {
        'next': next_url,
        'show_error_modal': bool(messages.get_messages(request))  # Affiche modal si messages d'erreur
    })



@login_required(login_url='espace_candidat')  
@user_passes_test(is_candidat)
def espace_candidat(request):
    ...
    try:
        ...
        messages.error(request, "Aucune inscription trouvée pour votre compte")
        return redirect('home')
...
class TeamMemberDetailView(DetailView):
    """Vue détaillée pour un membre d'équipe."""
    model = TeamMember
